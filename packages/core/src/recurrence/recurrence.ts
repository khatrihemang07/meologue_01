...
import { computeNextOccurrence } from "./engine";
 ...
 * the Task is completed, rather than computed once and fixed as a
 * schedule." ../task-store.ts's advanceRecurring is the intended caller:
 * it calls this fresh on every completion, passing whatever `dateString`
 * and reference dates the Task currently holds, rather than caching a
 * RecurrenceRule anywhere between calls.
 *
 ...
 * regardless as the floor below which no occurrence is ever returned. One
 * date can't serve both jobs at once — RecurrenceReference (./rule.ts)
 * names them `dueDate` and `now` rather than collapsing them, and this
 * function's own signature is the one place that decomposition is
 * spelled out, so nothing downstream has to guess which date it received.
 *
 ...
 *
 * **Skipping missed occurrences.** Only a date strictly after
 * `reference.now` is ever returned — a yearly rule due 1 Jan 2025, not
 * completed until 1 Jul 2026 (eighteen months late), doesn't land on 1
 * Jan 2026 (already in the past relative to the completion) but on 1 Jan
 * 2027: two years out from the original due date, not one, because
 * ./engine.ts steps forward from the anchor one full interval at a time
 * until it clears `reference.now`, rather than adding exactly one
 * interval and stopping there.
 *
 ...
 * caller checks, the same discipline this function's own result type
 * carries forward.
 */
