...
|---|---|---|---|---|---|
| QA-01 | A recognised match is an `inline-block` span with 4px horizontal padding — it occupies width, so following glyphs shift | quick-add.md § the one fact | — | `todoist-captured` | #225 built the contenteditable this row requires (`task-title-editor.tsx`, a ProseMirror `EditorView`) and its own documented seam for a decoration plugin (`extraPlugins`), but implements no recognition/decoration itself — that's #226. Left `todoist-captured` rather than `built`: nothing here yet produces the span this row actually describes, and marking it built without one would be the exact `built`/`matched` conflation this ledger's own vocabulary section warns against. |
