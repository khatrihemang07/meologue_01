...
describe("ComposerPage", () => {
  ...
  // composer-resume.ts's own module-level memory, seeded into `resumeSeek`
  // at mount and cleared by the same `settleSeek`/Send paths the `?d=`/`?e=`
  // seeks above already use. Proven the same way those suites prove which
  // seek is actually in flight: whether `pagination.fetchMore` gets called
  // for a day that ISN'T loaded, versus settling trivially for one that IS.
  describe("returning to the remembered day (composer-resume.ts)", () => {
    function seekEntry(id: string, createdAt: string): EntryStoreOutletContext["entries"][number] {
      return {
        id,
        deviceId: "device-a",
    ...
    // Every test here reads or writes the module's own singleton memory
    // directly — reset it before and after each one so a test that forgot
    // to clear can't leak into the next, the same reason last-session.test.ts
    // clears `sessionStorage` on both sides.
    beforeEach(() => {
      clearComposerResumeDay();
    });
    afterEach(() => {
    ...
    it("seeds a seek from the remembered day when no ?d=/?e= param is present", async () => {
      writeComposerResumeDay("2020-01-01");
      const fetchMore = vi.fn();

      renderComposerPage({
        ...readyContext,
        // The remembered day (2020-01-01) isn't among the loaded Entries —
        // only a genuine seek in flight for it asks for an older page.
        entries: [seekEntry("1", "2026-08-18T10:00:00.000Z")],
        pagination: { hasMore: true, fetching: false, fetchMore },
      });
    ...
    it("an explicit ?d= wins over the remembered day", async () => {
      // Deliberately set to the day that's already loaded — if the
      // remembered day were what actually drove the seek, it would settle
      // immediately with no fetch at all, the same as the plain `?d=`
      // suite's own "clears ?d= once the target day is already loaded".
      writeComposerResumeDay("2026-08-18");
      const fetchMore = vi.fn();
      ...
      renderComposerPage(
        ...
        // Not loaded — only a seek genuinely keyed on the URL's own day
        // asks for an older page.
        "/?d=2020-01-01",
      );
    ...
    it("nothing remembered, no seek param: never fetches an older page on mount", async () => {
      const fetchMore = vi.fn();
      ...
      // Give History's own effects a chance to run before asserting the
      // negative — otherwise this would pass trivially before anything had
      // a chance to fire at all (the same pattern the `?d=`/`?e=` suites'
      // own "does not call fetchMore while a page is already in flight"
      // tests use).
      await waitFor(() => expect(screen.getByTestId("url-query")).toHaveTextContent(""));
      expect(fetchMore).not.toHaveBeenCalled();
    });

    it("remembers the topmost visible day while mounted", async () => {
      // jsdom never lays out real scroll geometry, so the topmost row is
      // always `flatItems[0]` (history.test.tsx's own comment) — here,
      // the oldest Entry, since `orderedEntries` hands History oldest-to-
      // newest reading order.
      const older = seekEntry("1", "2020-01-01T10:00:00.000Z");
      const newer = seekEntry("2", "2020-01-02T10:00:00.000Z");

      renderComposerPage({ ...readyContext, entries: [newer, older] });

      await waitFor(() => expect(readComposerResumeDay()).toBe("2020-01-01"));
    });

    it("records nothing once the topmost visible row already belongs to the newest day", async () => {
      writeComposerResumeDay("2019-12-31");
      const only = seekEntry("1", "2020-01-01T10:00:00.000Z");

      renderComposerPage({ ...readyContext, entries: [only] });

      await waitFor(() => expect(readComposerResumeDay()).toBeNull());
    });

    it("does not overwrite the remembered day while the seek back to it is still travelling", async () => {
      // The guard in `handleVisibleDayChange`. History reports the OLDEST
      // loaded day on the first render of any mount (jsdom has no scroll
      // geometry, so `topIndex` stays 0 and `flatItems` runs oldest-first).
      // On a restoring visit that report arrives while the seek is still
      // paging backwards towards the remembered day — and writing it would
      // erase the very answer the seek exists to reach, before it gets
      // there. `hasMore: true` with a mocked `fetchMore` holds the seek in
      // flight so the window this guards is the whole test.
      writeComposerResumeDay("2019-06-01");
      const pagination = { hasMore: true, fetching: false, fetchMore: vi.fn() };

      renderComposerPage({
        ...readyContext,
        entries: [
          seekEntry("2", "2020-01-02T10:00:00.000Z"),
          seekEntry("1", "2020-01-01T10:00:00.000Z"),
        ],
        pagination,
      });

      await waitFor(() => expect(pagination.fetchMore).toHaveBeenCalled());
      // Unchanged — and specifically NOT "2020-01-01", the oldest loaded day
      // that History reported on mount.
      expect(readComposerResumeDay()).toBe("2019-06-01");
    });

    it("Send clears the remembered day", async () => {
      writeComposerResumeDay("2020-01-01");
      const referred: EntryStoreOutletContext["entries"][number] = {
        id: "referred-1",
        deviceId: "device-a",
        body: "hello",
        createdAt: "now",
      ...
      renderComposerPage({ ...readyContext, entries: [referred] });

      // Dirties the Composer via a real transaction (`insertAtCursor`),
      // the same "committing an edit" suite's own mechanism above — Send
      // is then an ordinary button click on already-dirtied content.
      swipeLeft(screen.getByText("hello"));
      fireEvent.click(await screen.findByText("Refer to this Entry"));
      fireEvent.click(screen.getByLabelText("Send"));
